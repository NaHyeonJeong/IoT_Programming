...
Node *insertR(Node *node, void *k, void *v, int hit, int(*compare)(const void *key1, const void *key2)) { // 실질적인 삽입 과정
	if (node == NULL) {
	...
	if (compare((char *)k, (char *)node->key) < 0) {
	    node->left = insertR(node->left, k, v, hit, compare); // 부모노드 보다 작은 경우 왼쪽으로
	}
	else if (compare((char *)k, (char *)node->key) > 0) {
    	node->right = insertR(node->right, k, v, hit, compare); // 부모노트 보다 큰 경우 오른쪽으로
    }
	else {// 이미 노드가 있는 경우에는 value1과 hit만 바꿈
	    node->value1 = v;
	    node->value2 = hit;
	}
...
void insert(RBT *tree, void *k, void *v, int hit) { // 삽입이 필요한 곳에서 호출할 때 사용
	tree->root = insertR(tree->root, k, v, hit, tree->compare);
